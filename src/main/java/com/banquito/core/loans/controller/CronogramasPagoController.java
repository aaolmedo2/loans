...
import com.banquito.core.loans.DTO.CronogramasPagoDTO;
import com.banquito.core.loans.service.CronogramasPagoService;
...
public class CronogramasPagoController {
    ...
    @Operation(summary = "Obtener todos los cronogramas de pagos")
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "204", description = "No hay cronogramas de pagos", content = @Content)
    })
    @GetMapping
    public ResponseEntity<List<CronogramasPagoDTO>> obtenerTodos() {
        log.info("Obteniendo todos los cronogramas de pagos");
        List<CronogramasPagoDTO> cronogramas = this.cronogramasPagoService.obtenerTodos();
    ...
    public ResponseEntity<List<CronogramasPagoDTO>> obtenerPorPrestamoCliente(
        ...
        List<CronogramasPagoDTO> cronogramas = this.cronogramasPagoService.obtenerPorPrestamoCliente(idPrestamoCliente);
        if (cronogramas.isEmpty()) {
            return ResponseEntity.noContent().build();
        }
        return ResponseEntity.ok(cronogramas);
    ...
    public ResponseEntity<List<CronogramasPagoDTO>> generarCronogramas(
        ...
        List<CronogramasPagoDTO> cronogramas = this.cronogramasPagoService.generarCronogramaPagos(idPrestamoCliente);
        return ResponseEntity.ok(cronogramas);
    }
}
